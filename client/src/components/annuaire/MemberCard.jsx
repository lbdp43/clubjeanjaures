...
function MemberCard({ member }) {
  ...
  const buildContactText = () => {
    ...
    lines.push(`Profil : ${window.location.origin}/annuaire/${member.id}`);
    return lines.join('\n');
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-3 sm:gap-4 mb-4">
        {member.photoUrl && (
          <img src={imgUrl(member.photoUrl, 400)} alt={member.companyName || 'Photo de profil'} loading="lazy" decoding="async" className="w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover flex-shrink-0" />
        )}
        {member.logoUrl ? (
          <img src={imgUrl(member.logoUrl, 400)} alt={member.companyName ? `Logo ${member.companyName}` : 'Logo entreprise'} loading="lazy" decoding="async" className="flex-1 min-w-0 max-w-[200px] sm:max-w-[260px] h-auto max-h-32 sm:max-h-40 rounded-2xl object-contain" />
        ) : (
      ...
      {(member.phone || email) && (
        <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-100">
          ...
          <button onClick={handleShare} className={`inline-flex items-center gap-1.5 text-xs sm:text-sm font-medium px-3 py-1.5 rounded-full transition-colors ${shared ? 'bg-green-100 text-green-700 border border-green-300' : 'border border-gray-200 text-text-muted hover:border-blue hover:text-blue'}`} aria-label="Partager la fiche">
            {shared ? (
              <>
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
